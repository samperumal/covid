...
export const store = new Vuex.Store({
	...
	mutations: {
		...
		setInitialComorbidities(state, newComorbidities) {
			for (var key in newComorbidities)
				Vue.set(state.data.initialAssessment.comorbidities, key, +newComorbidities[key])
		},
	...
	getters: {
      ...
      if (pScore < 4) { 
				...
				priorityScore.prioritisation = "Receive priority over all other groups in face of scarce resources."
			}
      else if (pScore < 6) {
				priorityScore.bucket = "orange"
				priorityScore.ventilator = "Intermediate priority for ventilator support."
				priorityScore.prioritisation = "Receive resources if available after all patients in red group allocated."
			}
      else if (pScore < 10) {
				priorityScore.bucket = "yellow"
				priorityScore.ventilator = "Low priority for ventilator support."
				priorityScore.prioritisation = "Receive resources if available after all patients in red and orange groups allocated."
			}
      else {
		...
		sofaScore: (state) => (sofa) => {
			...
			const respirationFraction = Math.round(+sofa.respPaO2 / (+sofa.respFiO2 / 100) * 100) / 100
